...
RegisterNetEvent('boombox:useBoombox')
AddEventHandler('boombox:useBoombox', function()
...
RegisterNetEvent('boombox:deleteObj', function(netId)
    if DoesEntityExist(NetToObj(netId)) then
        DeleteObject(NetToObj(netId))
        if not DoesEntityExist(NetToObj(netId)) then
            TriggerServerEvent('boombox:objDeleted')
        end
    end
end)

AddEventHandler('boombox:pickup', function()
    local ped = PlayerPedId()
    ...
    if xSound:soundExists(musicId) then
        TriggerServerEvent("boombox:soundStatus", "stop", musicId, {})
    end
    FreezeEntityPosition(closestRadio, false)
    TriggerServerEvent("boombox:deleteObj", ObjToNet(closestRadio))
    if activeRadios[closestRadio] then
        activeRadios[closestRadio] = nil
    end 
    TriggerServerEvent('boombox:syncActive', activeRadios)
    ClearPedTasks(ped)
end)
...
AddEventHandler('boombox:soundStatus', function(type, musicId, data)
    CreateThread(function()
        ...
        if type == "play" then
            TriggerServerEvent('boombox:DiscordKnows',data.link)
...
AddEventHandler('boombox:interact', function()
...
AddEventHandler('boombox:savedSongs', function(radio)
    savedSongsMenu(radio)
end)

AddEventHandler('boombox:saveSong', function()
    local input = lib.inputDialog('Save Song', {'Name', 'Youtube Link'})
    if input[1] and input[2] then
        TriggerServerEvent('boombox:save', input[1], input[2])
        lib.notify({
            title = 'Success',
...
AddEventHandler('boombox:selectSavedSong', function(data)
    selectSavedSong(data)
end)

AddEventHandler('boombox:playSavedSong', function(data)
    local musicId = 'id_'..data.id
    TriggerServerEvent("boombox:soundStatus", "play", musicId, { position = activeRadios[data.id].pos, link = data.link, volume = '0.2', distance = 25 })
...
AddEventHandler('boombox:deleteSong', function(data)
	...
	if confirmed == 'confirm' then
		TriggerServerEvent('boombox:deleteSong', data)
		lib.notify({
			title = 'Deleted',
...
AddEventHandler('boombox:playMenu', function(data)
    local musicId = 'id_'..data.id
    if data.type == 'play' then
        ...
        if keyboard then
            if keyboard[1] and tonumber(keyboard[2]) and tonumber(keyboard[2]) <= 40 and tonumber(keyboard[3]) and tonumber(keyboard[3]) <= 100 then
                TriggerServerEvent("boombox:soundStatus", "play", musicId, { position = activeRadios[data.id].pos, link = keyboard[1], volume = keyboard[3]/100, distance = keyboard[2] })
                activeRadios[data.id].data = {playing = true, currentId = 'id_'..PlayerId()}
                TriggerServerEvent('boombox:syncActive', activeRadios)
            end
        end
    ...
    elseif data.type == 'volume' then
        ...
        if keyboard then
            if tonumber(keyboard[1]) and tonumber(keyboard[1]) <= 100 then
                TriggerServerEvent("boombox:soundStatus", "volume", musicId, {volume = keyboard[1]/100})
            end
        end
    elseif data.type == 'distance' then
        ...
        if keyboard then
            if tonumber(keyboard[1]) and tonumber(keyboard[1]) <= 40 then
                TriggerServerEvent("boombox:soundStatus", "distance", musicId, {distance = keyboard[1]})
            end
        end
    end
end)
...
AddEventHandler('boombox:syncActive', function(activeBoxes)
